...
class ApprovalGateway:
    ...
    async def create_request(
        ...
        if tool_policy.classify(action_type) != "external":
            raise ValueError(
                f"Approval hanya boleh dibuat untuk aksi eksternal terklasifikasi: {action_type}"
            )
    ...
    async def approve_request(
        ...
        async with db.transaction() as conn:
            ...
            if not fingerprinter.verify_hash(row["action_type"], params, row["parameter_hash"]):
                ...
                return False, (
                    "Parameter aksi telah berubah sejak diajukan! "
                    "Izin lama otomatis dibatalkan."
                )
    ...
    async def reject_request(approval_id: str, expected_group_id: str | None = None) -> bool:
        async with db.transaction() as conn:
            cursor = await conn.execute(
                "SELECT group_id, status FROM approvals WHERE approval_id=?",
                (approval_id,),
            )
            raw = await cursor.fetchone()
            if not raw:
    ...
    async def execute_approved_request(approval_id: str) -> dict[str, Any]:
        async with db.transaction() as conn:
            ...
            if (
                ...
                and utc_now() > datetime.fromisoformat(row["expires_at"])
            ):
                await conn.execute(
                    "UPDATE approvals SET status=? WHERE approval_id=? AND status=?",
                    (
            ...
            if row["status"] != ApprovalStatus.APPROVED.value:
                return {
                    "success": False,
                    "error": f"APPROVAL_STATUS_{row['status'].upper()}",
                }

            execution_id = f"exec_{uuid.uuid4().hex[:12]}"
            claimed = await conn.execute(
                """UPDATE approvals SET status=?, execution_id=?
                   WHERE approval_id=? AND status=?""",
                (
        ...
        await db.execute(
            """UPDATE approvals SET status=?, execution_error=?
