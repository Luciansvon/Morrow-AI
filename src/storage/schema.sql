...
CREATE TABLE IF NOT EXISTS conversation_message_map (platform_message_id TEXT PRIMARY KEY, group_id TEXT NOT NULL, role_id TEXT, thread_id TEXT, task_id TEXT, root_user_text TEXT, response_text TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX IF NOT EXISTS idx_conversation_message_group ON conversation_message_map(group_id, created_at);
CREATE TABLE IF NOT EXISTS memories (id TEXT PRIMARY KEY, group_id TEXT NOT NULL DEFAULT '__global__', scope TEXT NOT NULL, role_id TEXT, user_id TEXT, key TEXT NOT NULL, value TEXT NOT NULL, memory_type TEXT NOT NULL DEFAULT 'fact', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_memories_user_unique ON memories(group_id, user_id, key) WHERE scope = 'user';
...
CREATE TABLE IF NOT EXISTS task_agent_runs (task_id TEXT NOT NULL, role_id TEXT NOT NULL, status TEXT NOT NULL, attempt_count INTEGER NOT NULL DEFAULT 0, response_text TEXT, error_text TEXT, started_at TIMESTAMP, finished_at TIMESTAMP, PRIMARY KEY (task_id, role_id), FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS approvals (approval_id TEXT PRIMARY KEY, group_id TEXT NOT NULL, action_type TEXT NOT NULL, normalized_parameters TEXT NOT NULL, parameter_hash TEXT NOT NULL, requested_by_role TEXT NOT NULL, idempotency_key TEXT NOT NULL UNIQUE, status TEXT NOT NULL DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, expires_at TIMESTAMP NOT NULL, approved_by TEXT, execution_id TEXT, execution_error TEXT, execution_owner_token TEXT, execution_lease_until REAL);
...
CREATE TABLE IF NOT EXISTS processed_events (event_id TEXT PRIMARY KEY, platform TEXT NOT NULL, group_id TEXT, status TEXT NOT NULL DEFAULT 'completed', attempt_count INTEGER NOT NULL DEFAULT 1, lease_until REAL, owner_token TEXT, last_error TEXT, processed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
