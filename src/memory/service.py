...
class MemoryService:
    ...
    async def set_memory(
        ...
        async with db.transaction() as conn:
            if existing:
                await conn.execute(
                    "UPDATE memories SET value=?, memory_type=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
                    (value, memory_type.value, mem_id),
                )
            else:
                await conn.execute(
                    "INSERT INTO memories (id, group_id, scope, role_id, key, value, memory_type) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ...
            await conn.execute(
                """INSERT INTO memory_audit
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    f"aud_{uuid.uuid4().hex[:10]}", mem_id, group_id, scope.value, role_val,
                    key, old_value, value, changed_by_actor,
                    changed_by_role.value if changed_by_role else None, reason,
                ),
            )

        return MemoryItem(
            id=mem_id, group_id=group_id, scope=scope, role_id=role_id,
    ...
    @staticmethod
    async def get_memory_audit_history(key: str, group_id: str = "__global__") -> list[dict[str, Any]]:
